function [curr_dat_sz, curr_lab_sz,curr_num_sz] = store2hdf5WithNums(filename, data, labels,nums, create, startloc, chunksz)  
  ...
  if create
    ...
    h5create(filename, '/imgNum', [num_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [num_dims(1:end-1) chunksz]); % width, height, channels, number 
    if ~exist('startloc','var') 
      startloc.dat=[ones(1,length(dat_dims)-1), 1];
      startloc.lab=[ones(1,length(lab_dims)-1), 1];
      startloc.num=[ones(1,length(num_dims)-1), 1];
    end 
  else  % append mode
    if ~exist('startloc','var')
        disp('appending')
      ...
      prev_num_sz=info.Datasets(3).Dataspace.Size;
      ...
      assert(prev_num_sz(1:end-1)==num_dims(1:end-1), 'num dimensions must match existing dimensions in dataset');
      startloc.dat=[ones(1,length(dat_dims)-1), prev_dat_sz(end)+1];
      startloc.lab=[ones(1,length(lab_dims)-1), prev_lab_sz(end)+1];
      startloc.num=[ones(1,length(num_dims)-1), prev_num_sz(end)+1];
  ...
  if ~isempty(data)
    ...
    h5write(filename, '/imgNum', single(nums), startloc.num, size(nums));  
  ...
  if nargout
    ...
    curr_num_sz=info.Datasets(3).Dataspace.Size;
  end
end
