...
for f=4:length(sequence_names)
    ...
    for j=2:length(imNames)%careful : ls has a / file name, which can be first or last depending on sort
       ...
       % str = sprintf('%s %f %f %f \n',imgSubPath ,featureData(1), featureData(2), featureData(3))
        ...
        skeleton = low_level_features(highLevelFeaturesLineNumber,:);
        %plot_skeletons(skeleton);
        %figure, imshow(img);
        %pause;
        %close all
        
        rightFoot = skeleton(43:45);
        rightKnee = skeleton(37:39);
        leftFoot = skeleton(40:42);
        leftKnee = skeleton(34:36);
        
        %
        leftKneeToRightKnee = rightKnee-leftKnee;
        
        if(leftKneeToRightKnee(3)>0.15)
            label=1
            lefts=lefts+1;
        elseif(leftKneeToRightKnee(3)<-0.15)
            label=0
            rights=rights+1;
        else 
            continue;
...
%         else
            ...
            if(isempty(trainLabel))
                ...
                trainLabel = cat(2, trainLabel, label);
            end
      %  end
    end     
