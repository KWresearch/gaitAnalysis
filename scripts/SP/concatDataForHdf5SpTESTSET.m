...
meanTrainingImage = computeMeanImageSP(sequencesForTesting);
...
for v=1:length(sequencesForTesting)%f=1:length(sequence_names)
    f= sequencesForTesting(v)
    sequenceName = sequence_names{f}
    
    sequencePath=strcat(img_dir,'/', sequenceName);
    sequenceDir=[sequencePath];
    imsPath=strcat(sequencePath, '/quickFilled/extractedFGMeanBG');
    imsDir = [imsPath];
   ...
   % [shuffledTrainImgData, shuffledTrainLabel ]=shuffleData(trainImgData, trainLabel );
    createHdf5SetSP(sequenceName, [], [] ,testImgData,testLabel, [], [])
    setNum=setNum+1;
    trainImgData=[];
    trainLabel=[];
    valImgData=[];
